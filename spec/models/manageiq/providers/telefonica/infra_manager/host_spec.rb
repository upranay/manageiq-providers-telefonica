...
telefonica-keystone:                     active
    ...
    let(:host) do
      FactoryGirl.create(:host_telefonica_infra).tap do |host|
        ...
        host.system_services << FactoryGirl.create(:system_service, :name => 'telefonica-nova-api')
        ...
        glance = FactoryGirl.create(:host_service_group_telefonica, :name => 'Glance services')
        # create glance_api and add it to both Host and HostServiceGroupTelefonica objects
        glance_api = FactoryGirl.create(:system_service, :name => 'telefonica-glance-api')
        glance.system_services << glance_api
        host.system_services << glance_api
        # create glance_registry and add it to Host object only
        glance_registry = FactoryGirl.create(:system_service, :name => 'telefonica-glance-registry')
        ...
        host.system_services << FactoryGirl.create(:system_service, :name => 'telefonica-keystone')
      end
    end

    before do
      FactoryGirl.create(:host_service_group_telefonica, :host => host, :name => 'Keystone')
    end
  ...
  describe "Overriden auth methods for ssh fleecing," do
    let(:ext_management_system) do
      ...
      FactoryGirl.create(:ems_telefonica_infra, :zone => zone).tap do |ems|
  ...
  describe "ironic tasks" do
    let(:ext_management_system) do
      _guid, _server, zone = EvmSpecHelper.create_guid_miq_server_zone
      FactoryGirl.create(:ems_telefonica_infra, :zone => zone)
    end
