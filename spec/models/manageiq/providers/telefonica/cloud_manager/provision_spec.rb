describe ManageIQ::Providers::Telefonica::CloudManager::Provision do
  ...
  let(:provider)     { FactoryGirl.create(:ems_telefonica_with_authentication) }
  let(:template)     { FactoryGirl.create(:template_telefonica, :ext_management_system => provider) }
  let(:user)         { FactoryGirl.create(:user) }
  let(:vm_telefonica) { FactoryGirl.create(:vm_telefonica, :ext_management_system => provider, :ems_ref => "6b586084-6c37-11e4-b299-56847afe9799") }
  let(:vm_prov)      { FactoryGirl.create(:miq_provision_telefonica, :userid => user.userid, :source => template, :request_type => 'template', :state => 'pending', :status => 'Ok', :options => options) }
  ...
  context "#prepare_for_clone_task" do
    ...
    context "security_groups" do
      let(:security_group_1) { FactoryGirl.create(:security_group_telefonica, :ems_ref => "340c315c-6c30-11e4-a103-56847afe9799") }
      let(:security_group_2) { FactoryGirl.create(:security_group_telefonica, :ems_ref => "41a73064-6c30-11e4-a103-56847afe9799") }
