describe ManageIQ::Providers::Telefonica::CloudManager::Vm do
  ...
  let(:terminated_vm) { FactoryGirl.create(:vm_telefonica) }
  ...
  context "#is_available?" do
    let(:ems) { FactoryGirl.create(:ems_telefonica) }
    let(:vm)  { FactoryGirl.create(:vm_telefonica, :ext_management_system => ems) }
  ...
  context "when resized" do
    ...
    let(:vm) { FactoryGirl.create(:vm_telefonica, :ext_management_system => ems, :cloud_tenant => cloud_tenant) }
    let(:flavor) { FactoryGirl.create(:flavor_telefonica, :ems_ref => '2') }
