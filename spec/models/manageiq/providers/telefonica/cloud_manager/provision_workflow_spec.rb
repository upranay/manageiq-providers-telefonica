describe ManageIQ::Providers::Telefonica::CloudManager::ProvisionWorkflow do
  ...
  let(:template) { FactoryGirl.create(:template_telefonica, :ext_management_system => provider) }
  ...
  context "without applied tags" do
    ...
    context "security_groups" do
      context "non cloud network" do
        it "#get_targets_for_ems" do
          sg = FactoryGirl.create(:security_group_telefonica, :ext_management_system => provider.network_manager)
      ...
      context "cloud network" do
        it "#get_targets_for_ems" do
          ...
          sg_cn = FactoryGirl.create(:security_group_telefonica, :ext_management_system => provider.network_manager, :cloud_network => cn1)
  ...
  context "with applied tags" do
    ...
    before do
      ...
      2.times { FactoryGirl.create(:security_group_telefonica, :ext_management_system => provider.network_manager) }
      ...
      provider.flavors << FactoryGirl.create(:flavor_telefonica)
      provider.flavors << FactoryGirl.create(:flavor_telefonica)
  ...
  context "With a user" do
    ...
    context "With a Valid Template" do
      ...
      context "#allowed_instance_types" do
        let(:template) { FactoryGirl.create(:template_telefonica, :hardware => hardware, :ext_management_system => provider) }
        ...
        context "with regular hardware" do
          ...
          it "filters flavors too small" do
            flavor = FactoryGirl.create(:flavor_telefonica, :memory => 1.gigabyte, :root_disk_size => 1.terabyte)
            provider.flavors << flavor
            provider.flavors << FactoryGirl.create(:flavor_telefonica, :memory => 1.gigabyte, :root_disk_size => 1.megabyte) # Disk too small
            provider.flavors << FactoryGirl.create(:flavor_telefonica, :memory => 1.megabyte, :root_disk_size => 1.terabyte) # Memory too small
      ...
      context "with valid relationships" do
        it "#allowed_availability_zones" do
          az = FactoryGirl.create(:availability_zone_telefonica)
          provider.availability_zones << az
          expect(workflow.allowed_availability_zones).to eq(az.id => az.name)
        end

        it "#allowed_availability_zones with NULL AZ" do
          provider.availability_zones << az = FactoryGirl.create(:availability_zone_telefonica)
        ...
        it "#allowed_security_groups" do
          sg = FactoryGirl.create(:security_group_telefonica)
      ...
      context "availability_zone_to_cloud_network" do
        it "has one when it should" do
          subnet = FactoryGirl.create(:cloud_subnet_telefonica)
          FactoryGirl.create(:cloud_network_telefonica, :ext_management_system => provider.network_manager, :cloud_subnets => [subnet])

          expect(workflow.allowed_cloud_networks.size).to eq(1)
        end

        it "filters cloud networks without subnets" do
          FactoryGirl.create(:cloud_network_telefonica, :ext_management_system => provider.network_manager)
      ...
      context "#display_name_for_name_description" do
        let(:flavor) { FactoryGirl.create(:flavor_telefonica) }
      ...
      context "tenant filtering" do
        before do
          @ct1 = FactoryGirl.create(:cloud_tenant_telefonica)
          @ct2 = FactoryGirl.create(:cloud_tenant_telefonica)
          provider.cloud_tenants << @ct1
          provider.cloud_tenants << @ct2
        end
        ...
        context "cloud networks" do
          before do
            subnet1 = FactoryGirl.create(:cloud_subnet_telefonica)
            subnet2 = FactoryGirl.create(:cloud_subnet_telefonica)
            subnet3 = FactoryGirl.create(:cloud_subnet_telefonica)
            subnet4 = FactoryGirl.create(:cloud_subnet_telefonica)
            subnet5 = FactoryGirl.create(:cloud_subnet_telefonica)
            @cn1 = FactoryGirl.create(:cloud_network_private_telefonica,
                                      :cloud_tenant          => @ct1,
                                      :ext_management_system => provider.network_manager,
                                      :cloud_subnets         => [subnet1])
            @cn2 = FactoryGirl.create(:cloud_network_private_telefonica,
                                      :cloud_tenant          => @ct2,
                                      :ext_management_system => provider.network_manager,
                                      :cloud_subnets         => [subnet2])
            @cn3 = FactoryGirl.create(:cloud_network_public_telefonica,
                                      :cloud_tenant          => @ct2,
                                      :ext_management_system => provider.network_manager,
                                      :cloud_subnets         => [subnet3])

            @cn_shared        = FactoryGirl.create(:cloud_network_private_telefonica,
                                                   :shared                => true,
                                                   :cloud_tenant          => @ct2,
                                                   :ext_management_system => provider.network_manager,
                                                   :cloud_subnets         => [subnet4])
            @cn_public_shared = FactoryGirl.create(:cloud_network_public_telefonica,
                                                   :shared                => true,
                                                   :cloud_tenant          => @ct2,
                                                   :ext_management_system => provider.network_manager,
        ...
        context "security groups" do
          before do
            @sg1 = FactoryGirl.create(:security_group_telefonica)
            @sg2 = FactoryGirl.create(:security_group_telefonica)
        ...
        context "floating ip" do
          before do
            cloud_network_public   = FactoryGirl.create(:cloud_network_public_telefonica)
            cloud_network_public_2 = FactoryGirl.create(:cloud_network_public_telefonica)
            router                 = FactoryGirl.create(:network_router_telefonica,
                                                        :cloud_network => cloud_network_public)
            @cloud_network         = FactoryGirl.create(:cloud_network_private_telefonica,
                                                        :cloud_tenant => @ct2)
            @cloud_network_2       = FactoryGirl.create(:cloud_network_private_telefonica,
                                                        :cloud_tenant => @ct2)
            _subnet                = FactoryGirl.create(:cloud_subnet_telefonica,
