describe ManageIQ::Providers::Telefonica::CloudManager::CloudVolume do
  ...
  describe "instance linsting for attaching volumes" do
    let(:first_instance) { FactoryGirl.create(:vm_telefonica, :ext_management_system => ems, :ems_ref => "instance_0", :cloud_tenant => tenant) }
    let(:second_instance) { FactoryGirl.create(:vm_telefonica, :ext_management_system => ems, :ems_ref => "instance_1", :cloud_tenant => tenant) }
    let(:other_tenant) { FactoryGirl.create(:cloud_tenant_telefonica, :ext_management_system => ems) }
    let(:other_instance) { FactoryGirl.create(:vm_telefonica, :ext_management_system => ems, :ems_ref => "instance_2", :cloud_tenant => other_tenant) }
