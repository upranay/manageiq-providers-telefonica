...
describe TelefonicaRabbitEventMonitor do
  ...
  context "testing a connection" do
    ...
    it "raise exception on an unsuccessful test with bad credentials" do
      ...
      expect(@rabbit_connection).to receive(:close)
      expect { TelefonicaRabbitEventMonitor.test_connection(@options) }.to raise_error(MiqException::MiqInvalidCredentialsError)
    end
    ...
    it "raise exception on an unsuccessful test with unreachable hostname" do
      ...
      expect(@rabbit_connection).to receive(:close)
      expect { TelefonicaRabbitEventMonitor.test_connection(@options) }.to raise_error(MiqException::MiqHostError)
    end
    ...
    it "raise exception on an unsuccessful test with unexpected exception" do
      ...
      expect(@rabbit_connection).to receive(:close)
      expect { TelefonicaRabbitEventMonitor.test_connection(@options) }.to raise_error("Cannot connect to rabbit amqp")
    end
  end
